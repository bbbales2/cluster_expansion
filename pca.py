...
plt.legend(['Component 0', 'Component 1'], fontsize = 20)
plt.savefig('/home/bbales2/cluster_expansion/images/components.png', dpi = 72)
plt.show()

plt.semilogy(pca.explained_variance_, '*')
plt.savefig('/home/bbales2/cluster_expansion/images/explained_variance.png', dpi = 72)
plt.show()
#%%
...
plt.show()
#%%
for name, fit, pca in zip(dims, fits, pcas):#[('160', fit1), ('100', fit2), ('60', fit3), ('20', fit4)]:
    #w = fit.extract()['w']
    w = fit.extract()['w'].dot(pca.components_)
    means = w.mean(axis = 0)
    stds = w.std(axis = 0)
    ...
    #plt.legend(['Stan mean', 'LR'])
    #plt.gcf().set_size_inches((12, 12))
    plt.savefig('/home/bbales2/cluster_expansion/images/bayes_{0}.png'.format(name))
    plt.show()
    print fit.extract()['sigma'].mean(), fit.extract()['sigma'].std()
    ...
    plt.show()
#%%
for name, fit, pca, c in zip(dims, fits, pcas, ['r', 'g'])[:2]:#[('160', fit1), ('100', fit2), ('60', fit3), ('20', fit4)]:
    #w = fit.extract()['w']
    w = fit.extract()['w'].dot(pca.components_)
    means = w.mean(axis = 0)
    stds = w.std(axis = 0)
    plt.plot(means, c)
    #plt.plot(lr.coef_, 'b')
    plt.plot(means + 2 * stds, '--{0}'.format(c))
    plt.plot(means - 2 * stds, '--{0}'.format(c))
    #plt.legend(['Stan mean', 'LR'])
    #plt.gcf().set_size_inches((12, 12))
    #plt.savefig('/home/bbales2/cluster_expansion/images/bayes_{0}.png'.format(name))
...
plt.ylim(-0.05, 0.05)
plt.xlim(5, 25)
#plt.legend(["Dim = {0}".format(name) for name in dims[:2]], fontsize = 20)
plt.savefig('/home/bbales2/cluster_expansion/images/bayes_dim_reduction.png', dpi = 72)
#plt.gcf().set_size_inches((8, 8))
plt.show()
#%%
n1 = 40
